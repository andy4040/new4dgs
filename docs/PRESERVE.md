...
1. `new4dgs` Git remote에 push한 모든 commit. 원격 `https://github.com/andy4040/new4dgs`에 **코드 push 완료 및 commit 확인**. 데이터·checkpoint·렌더 결과는 Git에 포함되지 않으므로 별도로 보존해야 합니다.
...
5. 외부 원본 coffee_martini 데이터 및 calibration. 현재 `/workspace/datasets/coffee_martini`에 있으며 묶음에 포함하지 않습니다. 공식 배포본으로 재다운로드할 수 있습니다. 생성 fixture 영상은 `scripts/make_fixture.py`로 재생성하므로 묶음에서 제외합니다.
